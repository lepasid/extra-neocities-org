...
while :; do
    ...
    output=$(curl -s -H "Connection: keep-alive" -A "Neocities Unblocker by KINI/1.1" "https://neocities.org/browse?sort_by=last_updated&tag=&page=$current_page" | grep -Eo "https://[a-zA-Z0-9.-]+\.neocities\.org" | grep -v "www\.neocities\.org" | grep -v "blog\.neocities\.org")
